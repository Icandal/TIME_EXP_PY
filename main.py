...
class KeyHandler:
    ...
    def handle_space(self):
        ...
        if exp.state.waiting_for_initial_start and exp.initial_instruction_screen.is_active:
            ...
            if exp.instruction_screen.handle_event(pygame.event.Event(pygame.KEYDOWN, key=pygame.K_SPACE)):
    ...
    def _can_stop_point(self):
        ...
        exp = self.experiment
        return (not exp.instruction_screen.is_active and 
                not exp.state.waiting_for_instruction and
                not exp.state.waiting_for_initial_start and
                not exp.timing_screen.is_active and
                not exp.reproduction_task.is_active and
                exp.moving_point is not None and 
                exp.moving_point.is_moving and 
                not exp.moving_point.stopped_by_user and
                not exp.current_task.timing_estimation and
                not exp.current_task.reproduction_task)
    ...
    def _can_show_help(self):
        ...
        return (not exp.instruction_screen.is_active and 
                not exp.state.waiting_for_initial_start and 
                not exp.timing_screen.is_active and 
                not exp.reproduction_task.is_active)
...
class ScreenManager:
    ...
    def draw_reproduction_task(self):
        ...
        self.experiment.reproduction_task.draw(self.experiment.screen, self.experiment.fixation)
...
class Experiment:
    ...
    def setup_components(self):
        ...
        self.current_task = self.config.get_current_task_config(self.current_trial["task_type"])
        
        # Создаем сборщик данных
        self.data_collector = DataCollector(self.config.participant_id, self.progress_info["block_number"])
    ...
    def calculate_trajectory_parameters(self):
        ...
        if (self.trajectory_manager.current_trajectory is not None and 
            hasattr(self.trajectory_manager.current_trajectory, 'calculate_duration')):
            self.calculated_duration = self.trajectory_manager.current_trajectory.calculate_duration(self.assigned_speed)
    ...
    def create_moving_point(self):
        ...
        if self.trajectory_manager.current_trajectory is not None:
            self.moving_point = MovingPoint(
                ...
                occlusion_type=self.current_task.occlusion_type if self.current_task.occlusion_enabled else "none"
            )
    ...
    def setup_screens(self):
        """Настройка экранов"""
        self.instruction_screen = InstructionScreen(self.screen_width, self.screen_height)
        
        self.initial_instruction_screen = InstructionScreen(self.screen_width, self.screen_height)
    ...
    def print_current_trial_info(self):
        ...
        if self.current_trial["duration"] is not None:
            info_lines.insert(7, f"Назначенная длительность: {self.current_trial['duration']} мс")
    ...
    def handle_instruction_continue(self):
        ...
        self.data_collector.complete_trial(completed_normally=not self.moving_point.stopped_by_user)
    ...
    def draw_info_panel(self):
        ...
        if (self.state.waiting_for_instruction and 
            ...
            time_left = self.state.INSTRUCTION_DELAY - (pygame.time.get_ticks() - self.state.instruction_delay_timer)
            if time_left > 0:
                delay_info = font.render(f"Задержка: {time_left} мс", True, (255, 0, 0))
                self.screen.blit(delay_info, (self.screen_width - 150, self.screen_height - 25))
        
        # Отображение основной информации
        y_positions = [self.screen_height - 120, self.screen_height - 95, self.screen_height - 70, 
                      self.screen_height - 45, self.screen_height - 20]
    ...
    def update_moving_point(self, dt):
        ...
        if (not self.state.occlusion_started and 
            self.moving_point.occlusion_enabled and 
            not self.moving_point.is_visible):
        ...
        if self.moving_point.should_switch_to_next() and not self.state.waiting_for_instruction:
            self.handle_trajectory_completion(current_time)
    
    def _can_update_point(self):
        """Проверка возможности обновления точки"""
        return (not self.instruction_screen.is_active and 
                not self.state.waiting_for_initial_start and
                not self.timing_screen.is_active and
                not self.reproduction_task.is_active and
                self.moving_point is not None)
    ...
    def handle_trajectory_completion(self, current_time):
        ...
        handler_key = 'timing_estimation' if self.current_task.timing_estimation else \
                     'reproduction_task' if self.current_task.reproduction_task else 'default'
    ...
    def handle_reproduction_task(self, actual_duration, current_time):
        """Обработка задачи воспроизведения"""
        self.data_collector.record_trajectory_duration(actual_duration)
        assigned_duration = self.current_trial["duration"] if self.current_trial["duration"] is not None else actual_duration
