...
class ReproductionTask:
    def __init__(self, screen_width: int, screen_height: int):
        ...
        # Состояния задачи
        self.state = (
            "first_stimulus"  # first_stimulus, interval, second_stimulus, response
        )
        self.start_time = 0
    ...
    def handle_event(self, event: pygame.event.Event) -> bool:
        ...
        if self.is_active and event.type == pygame.KEYDOWN:
            if (
                ...
                self.response_time = pygame.time.get_ticks() - (
                    self.start_time
                    + self.first_stimulus_duration
    ...
    def update(self) -> None:
        ...
        if (
            self.state == "first_stimulus"
            and current_time >= self.first_stimulus_duration
        ):
            self.state = "interval"

        elif (
            self.state == "interval"
            and current_time >= self.first_stimulus_duration + self.interval_duration
        ):
            self.state = "response"
    ...
    def draw(self, screen: pygame.Surface, fixation_cross) -> None:
        ...
        if self.state == "first_stimulus":
            ...
            instruction_text = font.render(
                "Запомните длительность показа кружка", True, (0, 0, 0)
            )
