...
class TaskConfig:
    ...
    def __init__(self, name: str, fixation_shape: FixationShape, occlusion_enabled: bool = True, 
                 occlusion_type: str = "half", timing_estimation: bool = False, 
                 reproduction_task: bool = False, reproduction_duration: int = 2000):
        self.name = name
...
class BlockConfig:
    """Конфигурация блока"""
    def __init__(self, name: str, tasks_distribution: Dict[int, int], trajectories_category: str):
        """
    ...
    def generate_trial_sequence(self, available_speeds: List[float], available_durations: List[int]) -> List[Dict[str, Any]]:
        ...
        for task_type, count in self.tasks_distribution.items():
            for i in range(count):
                ...
                speed = random.choice(available_speeds) if task_type in [0, 1, 2] else None
                
                # Случайно выбираем длительность для задач с временем (2,3)
                duration = random.choice(available_durations) if task_type in [2, 3] else None
...
class ExperimentConfig:
    def __init__(self):
        ...
        self.tasks: List[TaskConfig] = [
            TaskConfig("Задача 1: Окклюзия (половина)", FixationShape.TRIANGLE, True, "half"),
            TaskConfig("Задача 2: Без окклюзии", FixationShape.RHOMBUS, False, "half"),
            TaskConfig("Задача 3: Оценка времени (без окклюзии)", FixationShape.STAR, False, "half", True),
            TaskConfig("Задача 4: Воспроизведение времени", FixationShape.CROSS, False, "half", False, True, 2000)
        ]
        
        # Определяем блоки (8 блоков с разными настройками)
        self.blocks: List[BlockConfig] = [
            BlockConfig("Блок 1: Простые траектории", {0: 5, 1: 5, 2: 5, 3: 5}, "T"),
            ...
            BlockConfig("Блок 6: Короткие траектории 1", {0: 5, 1: 5, 2: 5, 3: 5}, "S1"),
            BlockConfig("Блок 7: Короткие траектории 2", {0: 5, 1: 5, 2: 5, 3: 5}, "S2"),
            BlockConfig("Блок 8: Смешанные траектории", {0: 5, 1: 5, 2: 5, 3: 5}, "T")
        ]
    ...
    def get_current_task_config(self, task_index: int) -> TaskConfig:
        ...
        if 0 <= task_index < len(self.tasks):
            ...
            return TaskConfig("Задача по умолчанию", FixationShape.TRIANGLE, True, "half")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "blocks": [{"name": block.name, "tasks_distribution": block.tasks_distribution, 
                       "trajectories_category": block.trajectories_category} for block in self.blocks]
        }
    ...
    def from_dict(self, config_dict: Dict[str, Any]) -> None:
        ...
        if isinstance(color_data, (list, tuple)) and len(color_data) >= 3:
            self.fixation_color = (int(color_data[0]), int(color_data[1]), int(color_data[2]))
        else:
        ...
        for task_data in tasks_data:
            task = TaskConfig(
                ...
                fixation_shape=FixationShape(task_data.get("fixation_shape", "triangle")),
        ...
        for block_data in blocks_data:
            block = BlockConfig(
                ...
                tasks_distribution=block_data.get("tasks_distribution", {0: 5, 1: 5, 2: 5, 3: 5}),
