...
class BlockManager:
    def __init__(self, trajectories_data: Dict[str, Any], blocks: List[BlockConfig], 
        ...
        for block in blocks:
            ...
            for trial in trial_sequence:
                ...
                trajectory_idx, actual_category = self._get_next_available_trajectory(category)
                trial["trajectory_index"] = trajectory_idx
                trial["actual_trajectory_category"] = actual_category  # Сохраняем реальную категорию
    ...
    def _get_next_available_trajectory(self, requested_category: str) -> tuple[int, str]:
        ...
        if requested_category == "R":
            ...
            available_categories = self._get_available_categories()
            for category in random.sample(available_categories, len(available_categories)):
                if category != actual_category:
                    trajectory_idx, new_category = self._get_next_available_trajectory(category)
        ...
        print(f"ВНИМАНИЕ: Все траектории категории {actual_category} использованы, сбрасываем...")
        
        # Удаляем все использованные траектории этой категории
        trajectories_to_remove = [key for key in self.used_trajectories_global if key[0] == actual_category]
    ...
    def get_progress_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "actual_trajectory_category": current_trial.get("actual_trajectory_category", current_block.trajectories_category),  # Реальная категория
    ...
    def get_used_trajectories_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "used_by_category": {
                category: len([key for key in self.used_trajectories_global if key[0] == category])
                for category in self.trajectories_data.keys() if category != "R"
            }
        }