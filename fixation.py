...
class FixationCross:
    ...
    def _draw_circle(
        ...
        pygame.draw.circle(
            screen, self.color, (center_x, center_y), self.size, 2
        )  # Обводка
    ...
    def _draw_star(self, screen: pygame.Surface, center_x: int, center_y: int) -> None:
        ...
        for i in range(10):
            angle = (
                2 * 3.14159 * i / 10 - 3.14159 / 2
            )  # Смещаем чтобы верхний луч был вверху
            radius = outer_radius if i % 2 == 0 else inner_radius
            x = (
                center_x
                + radius * pygame.math.Vector2(1, 0).rotate(angle * 180 / 3.14159).x
            )
            y = (
                center_y
                + radius * pygame.math.Vector2(1, 0).rotate(angle * 180 / 3.14159).y
            )
            points.append((x, y))
